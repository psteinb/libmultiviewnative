...
BOOST_AUTO_TEST_CASE(convolve_with_custom_one) {
  ...
  unsigned max_dim =
      *std::max_element(local_kernel1_dims.begin(), local_kernel1_dims.end());
  ...
  float sum_expected = std::accumulate(
      local_kernel1.stack_.data(),
      local_kernel1.stack_.data() + local_kernel1.stack_.num_elements(), 0.f);
  float sum_received = std::accumulate(
      local_one.data(), local_one.data() + local_one.num_elements(), 0.f);
  ...
  for (int i = 0; i < 3; ++i)
    expected_kernel_pos[i] = multiviewnative::range(
        local_one.shape()[i] / 2 - local_kernel1.stack_.shape()[i] / 2,
        local_one.shape()[i] / 2 - local_kernel1.stack_.shape()[i] / 2 +
            local_kernel1.stack_.shape()[i]);
  multiviewnative::image_stack_view kernel1_segment =
      local_one[boost::indices[expected_kernel_pos[0]][expected_kernel_pos[1]]
                              [expected_kernel_pos[2]]];
  multiviewnative::image_stack result = kernel1_segment;

  float l2norm = multiviewnative::l2norm(local_kernel1.stack_.data(),
...
BOOST_AUTO_TEST_CASE(convolve_with_custom_one_discrete) {
  ...
  // float sum_expected =
  // std::accumulate(local_kernel1.stack_.data(),local_kernel1.stack_.data()+local_kernel1.stack_.num_elements(),0.f);
  // float sum_received =
  // std::accumulate(local_one_fft.data(),local_one_fft.data()+local_one.num_elements(),0.f);

  image_stack local_one_discrete = local_one;
  multiviewnative::convolve(local_one, local_kernel1.stack_, local_one_discrete,
                            offsets_kernel1);

  float l2norm =
      multiviewnative::l2norm(local_one_fft.data(), local_one_discrete.data(),
                              local_one_fft.num_elements());
...
BOOST_AUTO_TEST_CASE(convolve_with_custom_one) {
  ...
  unsigned max_dim =
      *std::max_element(local_kernel2_dims.begin(), local_kernel2_dims.end());
  std::vector<int> image_dims(3, 3 * max_dim);
  ...
  float sum_expected = std::accumulate(
      local_kernel2.stack_.data(),
      local_kernel2.stack_.data() + local_kernel2.stack_.num_elements(), 0.f);
  float sum_received = std::accumulate(
      local_one.data(), local_one.data() + local_one.num_elements(), 0.f);
  ...
  for (int i = 0; i < 3; ++i)
    expected_kernel_pos[i] = multiviewnative::range(
        local_one.shape()[i] / 2 - local_kernel2.stack_.shape()[i] / 2,
        local_one.shape()[i] / 2 - local_kernel2.stack_.shape()[i] / 2 +
            local_kernel2.stack_.shape()[i]);
  multiviewnative::image_stack_view kernel2_segment =
      local_one[boost::indices[expected_kernel_pos[0]][expected_kernel_pos[1]]
                              [expected_kernel_pos[2]]];
  multiviewnative::image_stack result = kernel2_segment;

  float l2norm = multiviewnative::l2norm(local_kernel2.stack_.data(),
                                         result.data(), result.num_elements());
  BOOST_CHECK_LT(l2norm, 1.e-5);
}
...
BOOST_AUTO_TEST_CASE(convolve_with_custom_one_compare_to_gpu) {
  ...
  for (int i = 0; i < 3; ++i)
    expected_kernel_pos[i] = multiviewnative::range(
        cpu_local_one.shape()[i] / 2 - local_kernel2.stack_.shape()[i] / 2,
        cpu_local_one.shape()[i] / 2 - local_kernel2.stack_.shape()[i] / 2 +
            local_kernel2.stack_.shape()[i]);
  multiviewnative::image_stack_view kernel2_segment = cpu_local_one
      [boost::indices[expected_kernel_pos[0]][expected_kernel_pos[1]]
                     [expected_kernel_pos[2]]];
  multiviewnative::image_stack cpu_result = kernel2_segment;

  float l2norm =
      multiviewnative::l2norm(local_kernel2.stack_.data(), cpu_result.data(),
  ...
  kernel2_segment = gpu_local_one
      [boost::indices[expected_kernel_pos[0]][expected_kernel_pos[1]]
                     [expected_kernel_pos[2]]];
  multiviewnative::image_stack gpu_result = kernel2_segment;

  l2norm =
      multiviewnative::l2norm(local_kernel2.stack_.data(), gpu_result.data(),
                              gpu_result.num_elements());
  BOOST_CHECK_LT(l2norm, 1.e-5);
  std::cout << "gpu vs. expectation:\t" << l2norm << "\n";

  l2norm = multiviewnative::l2norm(cpu_result.data(), gpu_result.data(),
                                   gpu_result.num_elements());
  BOOST_CHECK_LT(l2norm, 1.e-5);
