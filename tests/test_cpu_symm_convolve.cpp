...
BOOST_AUTO_TEST_CASE(identity_convolve) {
  ...
  inplace_cpu_convolution(padded_image_.data(), &padded_image_dims_[0],
                          identity_kernel_.data(), &kernel_dims_[0], 1);
...
BOOST_AUTO_TEST_CASE(horizontal_convolve) {
  ...
  float sum_original =
      std::accumulate(image_folded_by_horizontal_.origin(),
                      ...
                      0.f);
  inplace_cpu_convolution(padded_image_.data(), &padded_image_dims_[0],
                          horizont_kernel_.data(), &kernel_dims_[0], 1);
...
BOOST_AUTO_TEST_CASE(vertical_convolve) {
  ...
  float sum_original =
      std::accumulate(image_folded_by_vertical_.origin(),
                      ...
                      0.f);
  inplace_cpu_convolution(padded_image_.data(), &padded_image_dims_[0],
                          vertical_kernel_.data(), &kernel_dims_[0], 1);
...
BOOST_AUTO_TEST_CASE(depth_convolve) {
  ...
  float sum_original = std::accumulate(
      ...
      0.f);
  inplace_cpu_convolution(padded_image_.data(), &padded_image_dims_[0],
                          depth_kernel_.data(), &kernel_dims_[0], 1);
...
BOOST_AUTO_TEST_CASE(all1_convolve) {
  ...
  float sum_original = std::accumulate(
      image_folded_by_all1_.origin(),
      image_folded_by_all1_.origin() + image_folded_by_all1_.num_elements(),
      0.f);
  inplace_cpu_convolution(padded_image_.data(), &padded_image_dims_[0],
                          all1_kernel_.data(), &kernel_dims_[0], 1);
...
BOOST_FIXTURE_TEST_SUITE(parallel_convolution_works,
