...
BOOST_AUTO_TEST_CASE( loaded_data_is_of_same_size )
{
  ...
  for (int i = 1; i < PaddedReferenceData::size; ++i)
    {
      BOOST_CHECK(local_ref.views_[i].image_dims_[0] == local_ref.views_[i-1].image_dims_[0]);
      BOOST_CHECK(local_ref.views_[i].image_dims_[1] == local_ref.views_[i-1].image_dims_[1]);
      BOOST_CHECK(local_ref.views_[i].image_dims_[2] == local_ref.views_[i-1].image_dims_[2]);
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(local_ref.views_[0].image_dims_.begin(), local_ref.views_[0].image_dims_.end(),
...
BOOST_AUTO_TEST_CASE( check_1_iteration )
{
  ...
  float l2norm = multiviewnative::l2norm(input_psi.data(), 
					 padded_psi_1->data(), 
