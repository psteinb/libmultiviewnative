...
BOOST_AUTO_TEST_CASE( reconstruct_anything_2iterations )
{
  ...
  try{
    ...
    write_image_stack(psi,"./reconstruct_anything_psi01.tif");
    std::cout << "max(received)["<< psi.num_elements()<<"] " << *std::max_element(psi.data(),psi.data() + psi.num_elements()) 
	      << ", max(expected)["<< local_guesses.psi(1)->num_elements()<<"]: " << *std::max_element(local_guesses.psi(1)->data(),local_guesses.psi(1)->data() + local_guesses.psi(1)->num_elements()) 
	      << "\n";
  }
  //tear-down
  delete [] input.data_;
