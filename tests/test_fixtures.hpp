...
public:
    ...
    asymm_identity_kernel_		(boost::extents[KernelDimSize+1][KernelDimSize][KernelDimSize-1])
  {
    ...
    for(int i = 0;i<3;++i){
      offsets[i] = halfKernel;
    }
    
    convolve(padded_image_, horizont_kernel_, padded_image_folded_by_horizontal, offsets);
    convolve(padded_image_, vertical_kernel_, padded_image_folded_by_vertical, offsets);
    convolve(padded_image_, depth_kernel_, padded_image_folded_by_depth, offsets);
    convolve(padded_image_, all1_kernel_, padded_image_folded_by_all1, offsets);
