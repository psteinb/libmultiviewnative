...
public:
    ...
    asymm_ranges_			    (n_dim)
  {
    ...
    for(int z_index = 0;z_index<int(asymm_cross_kernel_.shape()[0]);++z_index){
      for(int y_index = 0;y_index<int(asymm_cross_kernel_.shape()[1]);++y_index){
    	for(int x_index = 0;x_index<int(asymm_cross_kernel_.shape()[2]);++x_index){
	  
	  if(z_index == (int)asymm_cross_kernel_.shape()[0]/2 && y_index == (int)asymm_cross_kernel_.shape()[1]/2){
	    asymm_cross_kernel_[z_index][y_index][x_index] = x_index + 1;
	    asymm_one_kernel_[z_index][y_index][x_index] = 1;
	  }
	  
	  if(x_index == (int)asymm_cross_kernel_.shape()[2]/2 && y_index == (int)asymm_cross_kernel_.shape()[1]/2){
	    asymm_cross_kernel_[z_index][y_index][x_index] = z_index + 101;
	    asymm_one_kernel_[z_index][y_index][x_index] = 1;
	  }
	  
	  if(x_index == (int)asymm_cross_kernel_.shape()[2]/2 && z_index == (int)asymm_cross_kernel_.shape()[0]/2){
    ...
    auto div_by_2 = std::bind(std::divides<unsigned>(),std::placeholders::_1,2);
    std::transform(asymm_cross_kernel_.shape(),asymm_cross_kernel_.shape() + image_stack::dimensionality,
		   asymm_offsets_.begin(),
		   div_by_2);
    ...
    image_stack_view asymm_padded_image_original = asymm_padded_image_[ boost::indices[asymm_ranges_[0]][asymm_ranges_[1]][asymm_ranges_[2]] ];
    ...
    //CONVOLVE
    convolve(padded_image_, horizont_kernel_, padded_image_folded_by_horizontal, symm_offsets_);
    convolve(padded_image_, vertical_kernel_, padded_image_folded_by_vertical, symm_offsets_);
    convolve(padded_image_, depth_kernel_, padded_image_folded_by_depth, symm_offsets_);
    convolve(padded_image_, all1_kernel_, padded_image_folded_by_all1, symm_offsets_);

    convolve(asymm_padded_one_  ,  asymm_cross_kernel_     ,  asymm_padded_one_folded_by_asymm_cross_kernel     ,  asymm_offsets_);
    convolve(asymm_padded_one_  ,  asymm_one_kernel_       ,  asymm_padded_one_folded_by_asymm_one_kernel       ,  asymm_offsets_);
    convolve(asymm_padded_one_  ,  asymm_identity_kernel_  ,  asymm_padded_one_folded_by_asymm_identity_kernel  ,  asymm_offsets_);
    ...
    one_folded_by_asymm_cross_kernel_       = asymm_padded_one_folded_by_asymm_cross_kernel     [ boost::indices[asymm_ranges_[0]][asymm_ranges_[1]][asymm_ranges_[2]] ];
    one_folded_by_asymm_one_kernel_         = asymm_padded_one_folded_by_asymm_one_kernel       [ boost::indices[asymm_ranges_[0]][asymm_ranges_[1]][asymm_ranges_[2]] ];
    one_folded_by_asymm_identity_kernel_    = asymm_padded_one_folded_by_asymm_identity_kernel  [ boost::indices[asymm_ranges_[0]][asymm_ranges_[1]][asymm_ranges_[2]] ];
