...
struct divide_by_ {
  ...
  float operator()(const float& _in) { return _in / divisor; }
};
...
struct convolutionFixture3D {
 ...
 public:
  convolutionFixture3D()
    ...
    std::fill(trivial_kernel_.origin(), trivial_kernel_.origin() + kernel_size_,
              0.f);
    std::fill(identity_kernel_.origin(),
              identity_kernel_.origin() + kernel_size_, 0.f);
    std::fill(vertical_kernel_.origin(),
              vertical_kernel_.origin() + kernel_size_, 0.f);
    std::fill(depth_kernel_.origin(), depth_kernel_.origin() + kernel_size_,
              0.f);
    std::fill(horizont_kernel_.origin(),
              horizont_kernel_.origin() + kernel_size_, 0.f);
    ...
    for (int z_index = 0; z_index < int(asymm_cross_kernel_.shape()[2]);
         ++z_index) {
      for (int y_index = 0; y_index < int(asymm_cross_kernel_.shape()[1]);
           ++y_index) {
        for (int x_index = 0; x_index < int(asymm_cross_kernel_.shape()[0]);
             ++x_index) {

          if (z_index == (int)asymm_cross_kernel_.shape()[2] / 2 &&
          ...
          if (x_index == (int)asymm_cross_kernel_.shape()[0] / 2 &&
              y_index == (int)asymm_cross_kernel_.shape()[1] / 2) {
          ...
          if (x_index == (int)asymm_cross_kernel_.shape()[0] / 2 &&
              z_index == (int)asymm_cross_kernel_.shape()[2] / 2) {
    ...
    std::fill(padded_image_.origin(),
              padded_image_.origin() + padded_image_size, 0.f);
    ...
    for (int z_index = halfKernel;
         z_index < int(padded_image_axis - halfKernel); ++z_index) {
      for (int y_index = halfKernel;
           y_index < int(padded_image_axis - halfKernel); ++y_index) {
        for (int x_index = halfKernel;
             x_index < int(padded_image_axis - halfKernel); ++x_index) {
          ...
          for (int kindex = 0; kindex < KernelDimSize; ++kindex) {
            // convolution in x
            kernel_value = horizont_kernel_[KernelDimSize - 1 - kindex]
                                           [halfKernel][halfKernel];
            image_value =
                padded_image_[x_index - halfKernel + kindex][y_index][z_index];
            padded_image_folded_by_horizontal[x_index][y_index][z_index] +=
                kernel_value * image_value;

            // convolution in y
            kernel_value = vertical_kernel_
                [halfKernel][KernelDimSize - 1 - kindex][halfKernel];
            image_value =
                padded_image_[x_index][y_index - halfKernel + kindex][z_index];
            padded_image_folded_by_vertical[x_index][y_index][z_index] +=
                kernel_value * image_value;

            // convolution in z
            kernel_value = depth_kernel_[halfKernel][halfKernel]
                                        [KernelDimSize - 1 - kindex];
            image_value =
                padded_image_[x_index][y_index][z_index - halfKernel + kindex];
            padded_image_folded_by_depth[x_index][y_index][z_index] +=
                kernel_value * image_value;
          }

          newValue = 0.;
          for (int z_kernel = -(int)halfKernel; z_kernel <= ((int)halfKernel);
               ++z_kernel) {
            for (int y_kernel = -(int)halfKernel; y_kernel <= ((int)halfKernel);
                 ++y_kernel) {
              for (int x_kernel = -(int)halfKernel;
                   x_kernel <= ((int)halfKernel); ++x_kernel) {
                newValue +=
                    padded_image_[x_index + x_kernel][y_index + y_kernel]
                                 [z_index + z_kernel] *
                    all1_kernel_[halfKernel + x_kernel][halfKernel + y_kernel]
                                [halfKernel + z_kernel];
              }
            }
          }
...
BOOST_FIXTURE_TEST_SUITE(convolution_works,
                         multiviewnative::discrete_3D_fixture)
...
BOOST_AUTO_TEST_CASE(identity_convolve) {
  ...
  multiviewnative::convolve(image_, identity_kernel_, result, offsets);
...
BOOST_AUTO_TEST_CASE(horizont_convolve) {
  ...
  multiviewnative::convolve(image_, horizont_kernel_, result, offsets);

  float sum_expected =
      multiviewnative::sum_from_offset(image_folded_by_horizontal_, offsets);
  float sum_received = multiviewnative::sum_from_offset(result, offsets);
...
BOOST_AUTO_TEST_CASE(vertical_convolve) {
  ...
  multiviewnative::convolve(image_, vertical_kernel_, result, offsets);

  float sum_expected =
      multiviewnative::sum_from_offset(image_folded_by_vertical_, offsets);
  float sum_received = multiviewnative::sum_from_offset(result, offsets);
...
BOOST_AUTO_TEST_CASE(all1_convolve) {
  ...
  multiviewnative::convolve(image_, all1_kernel_, result, offsets);

  float sum_expected =
      multiviewnative::sum_from_offset(image_folded_by_all1_, offsets);
  float sum_received = multiviewnative::sum_from_offset(result, offsets);
...
BOOST_FIXTURE_TEST_SUITE(asymm_convolution_works,
                         multiviewnative::discrete_3D_fixture)
...
BOOST_AUTO_TEST_CASE(asymm_identity_convolve) {
  ...
  multiviewnative::convolve(image_, asymm_one_kernel_, result, offsets);
...
BOOST_AUTO_TEST_CASE(asymm_one_convolve_replicate) {
  ...
  image[(image_.shape()[0] - 1) / 2][(image_.shape()[1] - 1) / 2]
       [(image_.shape()[2] - 1) / 2] = 1.f;
  ...
  multiviewnative::convolve(image, asymm_one_kernel_, result, offsets);

  float sum_expected = std::accumulate(
      asymm_one_kernel_.data(),
      asymm_one_kernel_.data() + asymm_one_kernel_.num_elements(), 0.f);
  float sum_received = std::accumulate(
      result.data(), result.data() + result.num_elements(), 0.f);
...
BOOST_AUTO_TEST_CASE(asymm_cross_convolve_replicate) {
  ...
  multiviewnative::convolve(image, asymm_cross_kernel_, result, offsets);
