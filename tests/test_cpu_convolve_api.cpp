...
BOOST_AUTO_TEST_CASE(image_only_constructor) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(
      fold.image_->data(), fold.image_->data() + padded_image_.num_elements(),
      padded_image_.data(),
      padded_image_.data() + padded_image_.num_elements());

  BOOST_CHECK(std::equal(fold.extents_.begin(), fold.extents_.end(),
                         padded_image_dims_.begin()));
  BOOST_CHECK(!std::equal(fold.fft.fftw_shape()->begin(),
                          fold.fft.fftw_shape()->end(),
                          padded_image_dims_.cbegin()));
}
...
BOOST_AUTO_TEST_CASE(half_inplace_throws_due_to_wrong_kernel_setup) {
  ...
  // setup kernel
  std::vector<unsigned> padded_kernel_dims(kernel_dims_.begin(),
                                           kernel_dims_.end());
  padded_kernel_dims[2] = 2 * (padded_kernel_dims[2] / 2 + 1);
  mvn::fftw_image_stack padded_kernel(padded_kernel_dims);
  std::fill(padded_kernel.data(),
            padded_kernel.data() + padded_kernel.num_elements(), 0);
...
BOOST_AUTO_TEST_CASE(horizontal_convolve) {
  ...
  float sum_original =
      std::accumulate(image_folded_by_horizontal_.origin(),
                      image_folded_by_horizontal_.origin() +
  ...
  mvn::fftw_image_stack result = padded_image_
      [boost::indices[symm_ranges_[0]][symm_ranges_[1]][symm_ranges_[2]]];
...
BOOST_AUTO_TEST_CASE(vertical_convolve) {
  ...
  float sum_original =
      std::accumulate(image_folded_by_vertical_.origin(),
                      image_folded_by_vertical_.origin() +
...
BOOST_AUTO_TEST_CASE(depth_convolve) {
  ...
  float sum_original = std::accumulate(
      image_folded_by_depth_.origin(),
      image_folded_by_depth_.origin() + image_folded_by_depth_.num_elements(),
      0.f);
...
//     std::cout << "distance : " << distance_in_floats << " (n_el = " <<
// depth_kernel_.num_elements() << ")\n";
//   }
