...
const static multiviewnative::tiff_stack image_view_1("/scratch/steinbac/multiview_data/input_view_0.tif");
...
BOOST_AUTO_TEST_CASE( path_and_files_exists_images_have_right_extents )
{
  ...
  BOOST_CHECK_MESSAGE(!empty(), "image under " << location << " was not loaded" );
...
BOOST_AUTO_TEST_CASE( write_234_image )
{
  ...
  for (uint32_t i = 0; i < test.num_elements(); ++i)
    test.data()[i] = i;
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(test.data(), test.data() + test.num_elements() , 
