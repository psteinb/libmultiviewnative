...
BOOST_AUTO_TEST_CASE(first_value) { BOOST_CHECK_EQUAL(image_[0][0][0], 0); }

BOOST_AUTO_TEST_CASE(center_value) {
  using namespace multiviewnative;
  int center_one_dim_size = image_dims_[0] / 2;
  BOOST_CHECK_EQUAL(
      image_[center_one_dim_size][center_one_dim_size][center_one_dim_size],
      292);
}

BOOST_AUTO_TEST_CASE(axis_length) {
  BOOST_CHECK(image_.shape()[0] == 8 && image_.shape()[1] == 8 &&
              image_.shape()[2] == 8);
}
...
BOOST_AUTO_TEST_CASE(identity_kernely) {
  ...
  float sum = std::accumulate(identity_kernel_.data(),
                              identity_kernel_.data() + kernel_size_, 0.);
...
BOOST_AUTO_TEST_CASE(convolve_by_all1) {
  ...
  BOOST_CHECK_NE(
      image_[center_one_dim_size][center_one_dim_size][center_one_dim_size],
      image_folded_by_all1_[center_one_dim_size][center_one_dim_size]
                           [center_one_dim_size]);
  ...
  for (int z_shift = -1; z_shift <= 1; ++z_shift) {
    for (int y_shift = -1; y_shift <= 1; ++y_shift) {
      for (int x_shift = -1; x_shift <= 1; ++x_shift) {
        value +=
            image_[center_one_dim_size + x_shift][center_one_dim_size + y_shift]
                  [center_one_dim_size + z_shift];
      }
    }
  }
  BOOST_CHECK_EQUAL(
      value, image_folded_by_all1_[center_one_dim_size][center_one_dim_size]
                                  [center_one_dim_size]);
}
...
BOOST_AUTO_TEST_CASE(convolve_by_horizontal) {
  ...
  BOOST_CHECK_NE(
      image_[center_one_dim_size][center_one_dim_size][center_one_dim_size],
      image_folded_by_horizontal_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size]);

  float intermediate = 2 * image_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size] +
                       3 * image_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size - 1] +
                       1 * image_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size + 1];
...
BOOST_AUTO_TEST_CASE(convolve_by_vertical) {
  ...
  BOOST_CHECK_NE(
      image_[center_one_dim_size][center_one_dim_size][center_one_dim_size],
      image_folded_by_vertical_[center_one_dim_size][center_one_dim_size]
                               [center_one_dim_size]);

  float intermediate = 2 * image_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size] +
                       3 * image_[center_one_dim_size][center_one_dim_size - 1]
                                 [center_one_dim_size] +
                       1 * image_[center_one_dim_size][center_one_dim_size + 1]
                                 [center_one_dim_size];
...
BOOST_AUTO_TEST_CASE(convolve_by_depth) {
  ...
  BOOST_CHECK_NE(
      image_[center_one_dim_size][center_one_dim_size][center_one_dim_size],
      image_folded_by_depth_[center_one_dim_size][center_one_dim_size]
                            [center_one_dim_size]);

  float intermediate = 2 * image_[center_one_dim_size][center_one_dim_size]
                                 [center_one_dim_size] +
                       3 * image_[center_one_dim_size - 1][center_one_dim_size]
                                 [center_one_dim_size] +
                       1 * image_[center_one_dim_size + 1][center_one_dim_size]
                                 [center_one_dim_size];
...
BOOST_AUTO_TEST_CASE(convolve_by_asymm_cross_kernel) {
  ...
  float sum_received =
      std::accumulate(one_folded_by_asymm_cross_kernel_.data(),
                      one_folded_by_asymm_cross_kernel_.data() +
                          one_folded_by_asymm_cross_kernel_.num_elements(),
                      0.f);
...
BOOST_AUTO_TEST_CASE(convolve_by_asymm_one_kernel) {
  ...
  float sum_received =
      std::accumulate(one_folded_by_asymm_one_kernel_.data(),
                      one_folded_by_asymm_one_kernel_.data() +
                          one_folded_by_asymm_one_kernel_.num_elements(),
                      0.f);
...
BOOST_AUTO_TEST_CASE(convolve_by_asymm_identity_kernel) {
  ...
  float sum_expected = std::accumulate(
      ...
      0.f);
  float sum_received =
      std::accumulate(one_folded_by_asymm_identity_kernel_.data(),
                      one_folded_by_asymm_identity_kernel_.data() +
                          one_folded_by_asymm_identity_kernel_.num_elements(),
                      0.f);
