...
template <
    template <class> class TransformT = multiviewnative::inplace_3d_transform,
    typename PaddingT = no_padding,  // wrap_around_padding,
    typename TransferT = float, typename SizeT = unsigned>
struct cpu_convolve : public PaddingT {
  ...
  /**
     \brief the content of _image_stack_arr and _kernel_stack_arr is expected in
     row major memory order, however the extents are expected to contain the
     shape of the stack conforming this convention, so {z-dim, y-dim, x-dim}.
     ...
     \param[in] _image_stack_arr buffer containing an 3D image stack of shape
     _image_extents_arr
     \param[in] _kernel_extents_arr containing the shape of the kernel to
     receive
  ...
  */
  ...
  /**
     ...
     \param[in] _image_stack_arr buffer containing an 3D image stack of shape
     _image_extents_arr
     \param[in] _kernel_stack_arr buffer containing an 3D kernel stack of shape
     _kernel_extents_arr
  ...
  */
  ...
  cpu_convolve(value_type* _image_stack_arr, int_type* _image_extents_arr,
               ...
               int_type* _kernel_extents_arr = 0, size_type* _storage_order = 0)
      : PaddingT(&_image_extents_arr[0], &_kernel_extents_arr[0]),
        image_(0),
        padded_image_(0),
        kernel_(0),
  ...
  void inplace() {
    ...
    // shape_t tx_shape(padded_image_->shape(), padded_image_->shape() +
    // num_dims);
    ...
    complex_type* complex_kernel_fourier =
        (complex_type*)padded_kernel_->data();
    ...
    size_type transform_size =
        std::accumulate(this->extents_.begin(), this->extents_.end(), 1,
                        std::multiplies<size_type>());
    value_type scale = 1.0 / (transform_size);
    for (unsigned long index = 0; index < padded_image_->num_elements();
         ++index) {
    ...
    if (!std::equal(image_->shape(), image_->shape() + num_dims,
      ...
      (*image_) = (*padded_image_)
          [boost::indices[range(this->offsets_[0],
                                this->offsets_[0] + image_->shape()[0])]
                         [range(this->offsets_[1],
                                this->offsets_[1] + image_->shape()[1])]
  ...
  /**
     ...
     \param[in] _forwarded_padded_kernel forwarded kernel in the same data
  ...
  */
  void half_inplace(const fftw_image_stack& _forwarded_padded_kernel) {
    ...
    if (!std::equal(_forwarded_padded_kernel.shape(),
                    _forwarded_padded_kernel.shape() +
                        fftw_image_stack::dimensionality,
                    padded_image_->shape())) {

      std::ostringstream what_msg;
      auto printer = [&](const shape_iter_type& item) {
        what_msg << item << " ";
      };
      ...
      std::for_each(
          _forwarded_padded_kernel.shape(),
          _forwarded_padded_kernel.shape() + fftw_image_stack::dimensionality,
          printer);
      what_msg << ", expected: ";
      std::for_each(padded_image_->shape(),
                    padded_image_->shape() + fftw_image_stack::dimensionality,
                    printer);
    ...
    complex_type* complex_image_fourier = (complex_type*)padded_image_->data();
    complex_type* complex_kernel_fourier =
        (complex_type*)_forwarded_padded_kernel.data();
  ...
  ~cpu_convolve() {
    ...
    if (padded_image_) delete padded_image_;

    if (padded_kernel_) delete padded_kernel_;
  };
