...
static void HandleError(cudaError_t err, const char* file, int line) {
  if (err != cudaSuccess) {
    std::cerr << cudaGetErrorString(err) << " in " << file << " at line "
              << line << std::endl;
    cudaDeviceReset();
    exit(EXIT_FAILURE);
  }
...
#define HANDLE_NULL(a)                                                 \
  {                                                                    \
    if (a == NULL) {                                                   \
      std::cout << "Host memory failed in " << __FILE__ << " at line " \
                << __LINE__ << "\n";                                   \
      exit(EXIT_FAILURE);                                              \
    }                                                                  \
  }
...
void fit_2Dblocks_to_threads_for_device(dim3& _blocks, dim3& _threads,
                                        const int& _device) {
...
void print_3d_array(ArrT* _array, DimT* _array_dims, ODimT* _order) {
  std::cout << "received: " << _array_dims[0] << "x" << _array_dims[1] << "x"
            << _array_dims[2] << " array (" << _order[0] << ", " << _order[1]
            << ", " << _order[2] << ")\n";
  ...
  std::cout << "\n";
  std::cout << std::setfill('-') << std::setw((_array_dims[0] + 1) * 9) << " "
  ...
  for (index[2] = 0; index[2] < size_t(_array_dims[2]); ++index[2]) {
    ...
    for (index[1] = 0; index[1] < size_t(_array_dims[1]); ++index[1]) {
      ...
      for (index[0] = 0; index[0] < size_t(_array_dims[0]); ++index[0]) {
        ...
        flat_index =
            index[_order[0]] +
            _array_dims[_order[0]] *
                (index[_order[1]] + _array_dims[_order[1]] * index[_order[2]]);
...
template <typename ArrT, typename DimT, typename ODimT>
void print_device_array(ArrT* _device_address, DimT* _array_dims,
                        ODimT* _order) {

  size_t total =
      std::accumulate(_array_dims, _array_dims + 3, 1., std::multiplies<int>());
  ArrT* array = new ArrT[total];
  std::cout << "transferring memory from device -> host: " << total << "items ("
            << total * sizeof(ArrT) << "B)\n";
  HANDLE_ERROR(cudaMemcpy(array, _device_address, total * sizeof(ArrT),
