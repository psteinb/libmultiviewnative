...
class inplace_3d_transform {
 ...
 public:
  ...
  /**
     \brief
     forward r-2-c transform, the method assumes that the incoming ImageStackT
     has already been padded for the fft
  ...
  */
  void forward(ImageStackT* _input, plan_type* _plan = 0) {

    if (!std::equal(fftw_shape_.cbegin(), fftw_shape_.cend(),
                    _input->shape())) {
    ...
    if (!_plan) {
      if (!multiviewnative::plan_store<image_element_type>::get()->has_key(
               input_shape_)) {
        multiviewnative::plan_store<image_element_type>::get()->add(
            input_shape_);
      }
      _plan =
          multiviewnative::plan_store<image_element_type>::get()->get_forward(
              input_shape_);
    }
  ...
  void backward(ImageStackT* _input, plan_type* _plan = 0) {
    ...
    if (!_plan) {
      ...
      _plan =
          multiviewnative::plan_store<image_element_type>::get()->get_backward(
              input_shape_);
    }
  ...
  void padd_for_fft(ImageStackT* _input) {
    ...
    shape_type input_shape(_input->shape(),
                           _input->shape() + ImageStackT::dimensionality);

    if (!std::equal(fftw_shape_.cbegin(), fftw_shape_.cend(), _input->shape()))
      adapt_extents_for_fftw_inplace(input_shape, inplace_shape,
                                     _input->storage_order());
    else
      inplace_shape = fftw_shape_;

    _input->resize(
        boost::extents[inplace_shape[0]][inplace_shape[1]][inplace_shape[2]]);
  };

  void resize_after_fft(ImageStackT* _input) {

    _input->resize(
        boost::extents[input_shape_[0]][input_shape_[1]][input_shape_[2]]);
  };
  ...
  void generate_fftw_ready_shape(ImageStackT* _input,
                                 shape_type& _padded_shape) {

    std::vector<unsigned> padded_shape(ImageStackT::dimensionality, 0);
    std::vector<unsigned> input_shape(
        _input->shape(), _input->shape() + ImageStackT::dimensionality);
    adapt_extents_for_fftw_inplace(input_shape, _padded_shape,
                                   _input->storage_order());
  }

  shape_type const* fftw_shape() const { return &fftw_shape_; }
...
class parallel_inplace_3d_transform : public inplace_3d_transform<ImageStackT> {
 ...
 public:
  ...
  parallel_inplace_3d_transform() : inplace_3d_transform<ImageStackT>() {}
  ...
  static void set_n_threads(int _nthreads = 0) {
    ...
    if (_nthreads < 0) _nthreads = int(boost::thread::hardware_concurrency());

    if (_nthreads > 0) {
      parallel_inplace_3d_transform::nthreads_ = _nthreads;
      int success = 0;
      success =
          fftw_api_definitions<typename ImageStackT::element>::init_threads();
      if (!success) {
        std::cerr << "parallel_inplace_3d_transform::set_n_threads\tunable to "
                     "initialize threads of fftw3\n";
      }

      fftw_api_definitions<typename ImageStackT::element>::plan_with_threads(
          _nthreads);
    }
  }
