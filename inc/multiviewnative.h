...
// helper structs because Java clients are using JNA and we want to keep the API
// function signatures short
struct view_data {
...
/*
   Function for debug/benchmark purposes (not really needed in library
*/
FUNCTION_PREFIX void inplace_gpu_convolution(imageType* im, int* imDim,
                                             imageType* kernel, int* kernelDim,
                                             int device);
...
/*
   To be used for multi-view deconvolution
*/
FUNCTION_PREFIX void inplace_gpu_deconvolve(imageType* psi, workspace input,
                                            int device);
...
FUNCTION_PREFIX void convolution3DfftCUDAInPlace(imageType* im, int* imDim,
                                                 imageType* kernel,
                                                 int* kernelDim, int devCUDA);

FUNCTION_PREFIX void convolution3DfftCUDAInPlace_core(imageType* _d_imCUDA,
                                                      int* imDim,
                                                      imageType* _d_kernelCUDA,
                                                      int* kernelDim,
                                                      int devCUDA);

FUNCTION_PREFIX void compute_quotient(imageType* _input, imageType* _output,
                                      size_t _size, int _device);
FUNCTION_PREFIX void compute_final_values(imageType* _image,
                                          imageType* _integral,
                                          imageType* _weight, size_t _size,
                                          float _minValue, double _lambda,
                                          int _device);
FUNCTION_PREFIX void iterate_fft_plain(imageType* _input, imageType* _kernel,
                                       imageType* _output, int* _input_dims,
                                       int* _kernel_dims, int _device);

FUNCTION_PREFIX void iterate_fft_tikhonov(imageType* _input, imageType* _kernel,
                                          imageType* _output, int* _input_dims,
                                          int* _kernel_dims, size_t _size,
                                          float _minValue, double _lambda,
                                          int _device);
