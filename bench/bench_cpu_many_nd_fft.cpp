...
int main(int argc, char *argv[])
{
  ...
  desc.add_options()
    ...
    ("num_replicas,n", po::value<int>(&num_replicas)->default_value(4), "number of replicas to use for batched processing")
  ...
  const unsigned long total_pixel_count = num_replicas*aligned_input.num_elements();

  if(verbose) std::cout << "creating vector of " << total_pixel_count << " float elements\n";
  ...
  if(!batched_plan){		// 
    ...
    *global_plan = fftw_api::dft_r2c_3d(numeric_stack_dims[0], numeric_stack_dims[1], numeric_stack_dims[2],
					...
					(fftw_api::complex_type*)(aligned_input.data() ),
					FFTW_MEASURE);
    end = boost::chrono::high_resolution_clock::now();
    if(verbose)
      std::cout << "creating standard plan took (FFTW_MEASURE) " << boost::chrono::duration_cast<ms_t>(end - start) << "\n";
  } else {
    global_plan = new fftw_api::plan_type ;
    start = boost::chrono::high_resolution_clock::now();
    *global_plan = fftw_api::dft_r2c_many(numeric_stack_dims.size(), //int rank
    ...
    if(verbose)
      std::cout << "creating batched plan took (FFTW_MEASURE) " << boost::chrono::duration_cast<ms_t>(end - start) << "\n";
  }
  ...
  for(int r = 0;r<num_repeats;++r){
    ...
    start = boost::chrono::high_resolution_clock::now();
    if(!batched_plan){
      for(unsigned long count = 0;count < total_pixel_count;count+= aligned_input.num_elements()){
	reuse_fftw_plan(numeric_stack_dims,
			&many_inputs_flat[count],
			0,
			global_plan);
      }
    }
