...
struct simulated_data {
  ...
  CUDA_HOST_PREFIX simulated_data(const std::vector<int>& stack_dims,
                                  int _n_views = 6)
      : views_(_n_views),
        kernel1_(_n_views),
        kernel2_(_n_views),
  ...
  void info() {
    ...
    std::cout << "[simulated_data] " << nviews << "x views/weights "
...
void default_inplace_gpu_deconvolve(float* _psi, workspace* _ws,
                                    int device = -1) {

  unsigned long stack_size = _ws->data_[0].image_dims_[0] *
                             _ws->data_[0].image_dims_[1] *
                             _ws->data_[0].image_dims_[2];
  ...
  for (int it = 0; it < _ws->num_iterations_; ++it) {
    ...
    for (int v = 0; v < _ws->num_views_; ++v) {
      ...
      convolution3DfftCUDAInPlace(&integral[0], _ws->data_[v].image_dims_,
                                  _ws->data_[v].kernel1_,
                                  _ws->data_[v].kernel1_dims_, device);

      // serial
      computeQuotient(_ws->data_[v].image_, &integral[0], stack_size);

      convolution3DfftCUDAInPlace(&integral[0], _ws->data_[v].image_dims_,
                                  _ws->data_[v].kernel2_,
                                  _ws->data_[v].kernel2_dims_, device);
      // serial
      serial_regularized_final_values(_psi, &integral[0],
                                      _ws->data_[v].weights_, stack_size,
                                      _ws->lambda_, _ws->minValue_);
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()("help,h", "produce help message")(
      ...
      "use_default,d",
      "use implementation used in the original SPIM_Registration plugin ")(
      "stack_dimensions,s",
      po::value<std::string>(&stack_dims)->default_value("512x512x64"),
      "HxWxD of synthetic stacks to generate")(
      "views,n", po::value<int>(&num_views)->default_value(6),
      "number of views to generate")(
      "iterations,i", po::value<int>(&num_iterations)->default_value(5),
  ...
  std::vector<float> psi(data.views_[0].data(),
                         data.views_[0].data() + data.views_[0].num_elements());
  double time_ms = 0.f;
  ...
  for (int r = 0; r < num_repeats; ++r) {
    ...
    if (use_default) {
      if (verbose)
        std::cout << "[default inplace_gpu_deconvolve] repeat " << r << "/"
                  << num_repeats;
      cpu_timer timer;
      ...
      if (verbose)
        std::cout << "[inplace_gpu_deconvolve] repeat " << r << "/"
                  << num_repeats;
      cpu_timer timer;
    ...
    if (verbose) {
      std::cout << " took " << double(durations[r].system + durations[r].user) /
                                   1e6 << " ms\n";
    }
  }
