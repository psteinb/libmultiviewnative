...
int main(int argc, char* argv[]) {
  ...
  if (exp_mem_mb > av_mem_mb) {
    ...
    if (verbose)
      std::cout << "[NOT IMPLEMENTED YET] memory estimate: needed "
                << exp_mem_mb << " MB), available: " << av_mem_mb << " MB\n";
  }
  ...
  bool shapes_differ = !std::equal(numeric_stack_dims.begin(),
                                   numeric_stack_dims.end(), reshaped.begin());
  ...
  std::random_shuffle(aligned_input.data(),
                      aligned_input.data() + aligned_input.num_elements());
  ...
  if (use_global_plan) {
    ...
    *global_plan = fftw_api::dft_r2c_3d(
        ...
        (fftw_api::complex_type*)(out_of_place ? aligned_output->data()
                                               : aligned_input.data()),
        reuse_global_plan ? FFTW_MEASURE : FFTW_ESTIMATE);
    end = boost::chrono::high_resolution_clock::now();
    if (verbose)
      std::cout << "creating plan took ("
                << (reuse_global_plan ? "FFTW_MEASURE" : "FFTW_ESTIMATE")
  ...
  for (int r = 0; r < num_repeats; ++r) {
    ...
    if (!reuse_global_plan) {
      ...
      st_fftw(numeric_stack_dims, aligned_input.data(),
              out_of_place ? d_dest_buffer : 0,
              use_global_plan ? global_plan : 0);
    } else {
      if (verbose) std::cout << "[reuse_plan]\t";
      reuse_fftw_plan(numeric_stack_dims, aligned_input.data(),
